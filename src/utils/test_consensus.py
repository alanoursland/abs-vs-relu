...
def load_model(config, run):
    ...
    elif config.model == "WineQualityMLP" and config.dataset == "winequality_mlp":
...
def main(config_files):
    ...
    elif configs[0].dataset == "adult":
        _, test_loader = load_adult(batch_size=configs[0].test_batch_size)
    elif configs[0].dataset == "winequality":
        _, test_loader = load_winequality(batch_size=configs[0].test_batch_size)
    else:
